...
def pde(X_collocation, model_v, model_u_e, M_i, M_e):
    ...
    # Laplacians using non-isotropic conductivity tensors
    laplacian_v = M_i[0, 0] * v_xx + M_i[1, 1] * v_yy
    laplacian_u_e = M_e[0, 0] * u_e_xx + M_e[1, 1] * u_e_yy
...
def train_step(X_boundary, X_collocation, X_ic, optimizer, model_v, model_u_e, normal_vectors, M_i, M_e):
    optimizer.zero_grad()

    IC_loss = IC(X_ic, model_v, model_u_e)
    pde_loss = pde(X_collocation, model_v, model_u_e, M_i, M_e)
...
R = torch.tensor([[cos_theta, -sin_theta], [sin_theta, cos_theta]], dtype=torch.float32)

# Define the diagonal conductivity matrices for intracellular and extracellular spaces
D_i = torch.tensor([[1.0, 0.0], [0.0, 0.0]], dtype=torch.float32)
D_e = torch.tensor([[0.5, 0.0], [0.0, 0.0]], dtype=torch.float32)  # Example scaling for D_e
...
for epoch in range(epochs + 1):
    pde_loss, IC_loss, BC_loss, total_loss = train_step(X_boundary, X_collocation, X_ic, optimizer, model_v, model_u_e, normal_vectors, M_i, M_e)
...
plt.tight_layout()
#plt.savefig(f"figures/Comparison_PINNs_bidomain_different_time_points.pdf")
plt.show()