...
with an amplitude of 200. The conductivity tensor is set to be 1, so the current should travel equally in all directions
...
for pair in pairwise_time_points:
    ...
    # Set the main title
    #fig.suptitle(f"Numerical Solutions of the Bidomain Model for $x, y \\in [0,1]$", fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"figures_fem_mono/FEM_solution_for_corner_current_t{pair[0]}_t{pair[1]}.pdf")
    plt.show()


"""

Computation time (excluding visualization): 0.52 seconds
