...
print("Solving third simulation with FEM")
...
print("Solving third simulation with PINNs")
device = 'cpu'
...
plt.savefig(f"monodomain_results/pinns_corner_non_iso_loss_for_epochs={epochs}.pdf")
plt.close()
...
for t in time_points:
    ...
    plt.tight_layout()
    plt.savefig(f"monodomain_results/mono_current_noniso_comparison_at_{t}.pdf")
    plt.close()
...
for t in time_points:
    ...
    if numerical_solution_fem is not None:
        ...
        ax.set_ylabel('y')
        plt.tight_layout()
        plt.savefig(f"monodomain_results/error_non_iso_comparison_at_{t}.pdf")
        plt.close()
    else:
