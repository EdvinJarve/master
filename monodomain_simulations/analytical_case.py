...
# Save FEM data for later reference
...
# =============================================================================
# Optional scaling functions (not applied inside the PINN anymore)
# =============================================================================

def scale_to_m1p1(X):
    return 2.0 * X - 1.0

# If you want to apply scaling, you can do:
# X_collocation_scaled = scale_to_m1p1(X_collocation_tensor)
# ... and then use X_collocation_scaled in training steps.
# Otherwise, just use X_collocation_tensor directly.

# =============================================================================
# Define and Train PINN Model
# =============================================================================
...
params_file = os.path.join(base_results_dir, 'model_parameters.json')
...
best_model_path = os.path.join(base_results_dir, 'best_model.pth')
...
for epoch in range(epochs + 1):
    # If you want scaling:
    pde_loss, IC_loss, BC_loss, data_loss, ode_loss, total_loss = model.train_step(optimizer, batch_size, scaling_func=scale_to_m1p1)
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(base_results_dir, 'loss_plot.png'), dpi=300)
plt.close()
print("Loss plot saved.")

# Evaluate at FEM points at T=1.0 for test MSE
...
print(f"Test MSE at T={T}: {test_mse:.4e}")
...
for tt in comparison_times:
    ...
    plt.savefig(os.path.join(base_results_dir, plot_filename), dpi=300)
    plt.close()
